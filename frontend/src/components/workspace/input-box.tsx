...
export function InputBox({
  ...
  return (
    <div ref={promptRootRef} className="relative">
      <PromptInput
        className={cn(
          "rounded-2xl backdrop-blur-md transition-all duration-300 ease-out *:data-[slot='input-group']:rounded-2xl",
          className,
        )}
        disabled={disabled}
        globalDrop
      ...
      >
        ...
        {mounted && isNewThread && searchParams.get("mode") !== "skill" && (
          <div className="absolute right-0 -bottom-20 left-0 z-0 flex items-center justify-start px-4">
            <SuggestionList />
          </div>
        )}
        {!isNewThread && (
        ...
        (followupsLoading || followups.length > 0) && (
          <div className="absolute -top-20 right-0 left-0 z-20 flex items-center justify-center">
            <div className="flex items-center gap-2">
              {followupsLoading ? (
                <div className="glass-panel neon-border text-muted-foreground rounded-full px-4 py-2 text-xs backdrop-blur-md">
...
function SuggestionList() {
  ...
  useEffect(() => {
    ...
    const measure = () => setSuggestionsWidth(el.scrollWidth);
    measure();
  ...
  return (
    <div className="flex w-fit items-center gap-2">
      ...
      <div
        className={cn(
          "overflow-hidden transition-[width] duration-300 ease-out will-change-[width]",
        )}
        style={{ width: open ? suggestionsWidth : 0 }}
      >
        <div
          ref={suggestionsWrapRef}
          className={cn(
            "origin-left w-fit transform-gpu transition-[opacity,transform] duration-200 ease-out",
            open
              ? "animate-suggestions-open-right opacity-100 translate-x-0"
              : "pointer-events-none opacity-0 -translate-x-1",
          )}
        >
          <Suggestions className="min-h-16 w-fit origin-left items-start">
            <ConfettiButton
